...
// une autre maniere de faire l'injection de dependanse avec le constructeur .
  constructor(
  ...
  getInfoPatient(){
    this.router.navigate(['administrateur/form'])
  }
  getPatients(){ // Cette methode utilise le service HttpClient pour effectuer une requete GET vers l'URL 'http://localhost:4000/patients' et retourne un Observable qui contient la réponse de la requete. L'observable est ensuite utilisé dans la methode ngOnInit pour récupérer les données des patients et les stocker dans l'attribut Patients2.
    let patient = this.http.get('http://localhost:4000/patients');
     return patient;
  ...
  editPatient(data: any){
   console.log("test")
   this.router.navigate(['administrateur/edit-patient', data.id])
  }
